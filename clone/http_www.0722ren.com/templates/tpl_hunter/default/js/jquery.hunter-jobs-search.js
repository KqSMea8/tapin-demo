...
/*职位选择弹出层填充数据*/
function jobs_filldata(fillID, data_resourcesP, data_resources, resultlist, showID, resultshowID, resulthidId, dir) {
	...
	for (var i = 0; i < rows_city; i++) {
		...
		for (var j = 0; j < 5; j++) {
			if (data_resourcesP[subscriptnum_city]) {
				...
				if (citylist_array[1].length > 8) {
					citylist_array[1] = citylist_array[1].substring(0,8);
				};
	...
	$(fillID + " a.cat").unbind().on("click",function() {
		...
		if (data_resources[cityarrayid_array[0]]) { // 判断是否有二级地区
			...
			for (var i = 0; i < rows; i++) {
				...
				for (var j = 0; j <= 2; j++) {
					if (tcity_array[subscriptnum]) {
						...
						if (tcitylist_array[1].length > 8) {
							tcitylist_array[1] = tcitylist_array[1].substring(0,8);
						};
			...
			var n = $(this).next();
			pheight - dheight - iheight < comparetop ? (n.removeClass("sub").addClass("sup"), d.css({left:(pwidth-dwidth < dcleft ? (pwidth-dwidth-((pleft+pwidth)-(ileft+iwidth))) : dcleft),top:(comparetop + iheight) - bpv})) : (n.removeClass("sup").addClass("sub"), d.css({left:(pwidth-dwidth < dcleft ? (pwidth-dwidth-((pleft+pwidth)-(ileft+iwidth)))  : dcleft),top:(comparetop + iheight) - bpv}));
...
function allaround(dir) {
	...
	$(".saixuan").hover(function () {
        $(this).addClass("selectd").find(".saixuans").show();
    }, function () {
        $(this).removeClass("selectd").find(".saixuans").hide();
    });
	// 显示下拉
...
function search_location(dir) {
	...
	var page=1;
	$.get(dir+"plus/ajax_search_location.php", {"act":listype,"key":key,"jobcategory":jobcategory,"trade":trade,"citycategory":citycategory,"wage":wage,"age":age,"education":education,"experience":experience,"settr":settr,"scale":scale,"sort":sort,"page":page},
